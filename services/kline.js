...
async function processTickerPrice(tickerData) {
	...
	if (onEvaluateCandleSticks.length >= 2) {
		...
		if (
			...
			const avgPrice = (parseFloat(highestPrice) + parseFloat(lowestPrice)) / 2;

			// step 2: send FUTURE recommendation over facebook
			sendWhaleAlert({
				symbol,
...
function evaluateCandlesticks(symbol, candlesticks) {
	...
	console.log(
		`Symbol: ${symbol} - Previous candle is ${
			isPreviousCandleUp ? 'up' : 'down'
		} - Later candle is ${isLaterCandleUp ? 'up' : 'down'}`
	);
	console.log(
		'Percentage change:',
		percentageChange > 0
			? `later candle is higher than previous one: ${Math.abs(
					percentageChange
			  )}`
